...
const getHundredsBaseString = (num) => {
    if (num >= 100 && num <= 999) {
...
const getOnesBaseString = (num) => {
    if (num <= 19) {
        console.log("getOnesBaseString() cannot be called on numbers between 'X00' and 'X19'.");
        return '';
    }
    let numMinusHundreds = getNumMinusHundreds(num);
...
const getFullString = (maxNum) => {
    let finalString = '';
    for (let i = 1; i <= maxNum; i++) {
        const stringToAdd = convertToWords(i);
